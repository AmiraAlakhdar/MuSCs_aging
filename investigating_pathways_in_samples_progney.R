...
MuSCs <- progeny(seurat_integrated, scale=FALSE, organism="Mouse", top=500, perm=1, 
                return_assay = TRUE)
...
CellsClusters <- data.frame(Cell = names(Idents(seurat_integrated)), 
                            CellType = as.character(seurat_integrated@meta.data$sample),
                            check.names = F)
...
summarized_progeny_scores_df_2 <- summarized_progeny_scores %>%
  dplyr::select(-std) %>%   
  spread(Pathway, avg) %>%
